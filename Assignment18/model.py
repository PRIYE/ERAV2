...
# This function is required because LayerNormalization doen;t have bias =0
class LayerNormalization(nn.Module):
    ...
    def forward(self, x):
        ...
        #eps is added to care of the number hen the std is 0
...
class InputEmbeddings(nn.Module):
    def __init__(self, d_model:int, vocab_size: int) -> None:
        ...
        self.embedding = nn.Embedding(vocab_size, d_model) #ramdon initialisation
...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        #log and exp is for numerical stability
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        pe = pe.unsqueeze(0) # (1, seq_length, d_model)
        self.register_buffer('pe', pe)
        #pe is stored and not propagated
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        assert d_model % h == 0, "d_modelnot divisible by head"
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        d_k = query.shape[-1]
        #print(d_k.shape, query.shape, key.shape)
        ...
        if mask is not None:
            ...
            #wherever mask is 0 it is filled with small number
